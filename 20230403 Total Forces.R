...
R.Vector.Mag <- function(R.Vect) { #takes output of R.Vector as an input
  magnitude <- sqrt(sum(R.Vect^2))
  return(magnitude)
}

#DLC 1.3: Design Levels (single windspeed)
current.angle <- -30 #degrees
current.angle.rad <- deg2rad(current.angle) 
current.force <- 9.7 #kN
wind.force <- 2840 #kN
wave.angle <- 0
wave.force <- 4 #kN
...
Wind <- c(wind.force,0) #always a positive x direction vector
Current <- c(current.force*cos(current.angle.rad), current.force*sin(current.angle.rad))
Wave <- c(wave.force,0)
# Wave.high <- c(50,0)

V.1.3 <- R.Vector(Wind, Wave, Current)
V.1.3.mag <- R.Vector.Mag(V.1.3)
...
phi.rad <- deg2rad(phi.wave)

# Wave.low2.mag <- 8.2
# Wave.mag <- 0
# Wave <- c(0,0)
V.6.1 <- R.Vector(Wind,Wave,Current)
V.6.1.mag <- R.Vector.Mag(V.6.1)
for(phi in phi.rad) {
...
for(phi in phi.rad) {
  ...
  High.F <- R.Vector(Wind, Wave.high2, Current)
